...
namespace Com.TheFallenGames.OSA.Editor
{
	public abstract class BaseOSAWindow<TWindowParams> : EditorWindow where TWindowParams : BaseWindowParams
	{
		...
		{
			_Icon = AssetDatabase.LoadAssetAtPath("Assets/OSA/img/EditorOnly/osa-icon.png", typeof(Texture2D)) as Texture2D;
			_TopToBottomGradient = AssetDatabase.LoadAssetAtPath("Assets/OSA/img/EditorOnly/gradient.png", typeof(Texture2D)) as Texture2D;
		...
		protected void DrawIcon()
		{
			...
			newColor.a = .95f;
			GUI.color = newColor;
			GUI.Label(r, "OSA v" + C.OSA_VERSION_STRING, style);
			GUI.color = prevColor;
		}
		...
		protected void DisableOrNotifyAboutMiscComponents(GameObject go, string nameToUse, params System.Type[] typesToIgnore)
		{
			...
			foreach (var comp in components)
			{
				...
				Debug.Log("OSA: Found unknown component '" + mb.GetType().Name + "' on " + nameToUse + ". Make sure it doesn't interfere with OSA");
			}
		}
	}
