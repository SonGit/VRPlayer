...
#define OSA_SCROLLRECT_HAS_VIEWPORT
#endif
...
namespace Com.TheFallenGames.OSA.Editor
{
	public class InitOSAWindow : BaseOSAWindow<InitOSAWindow.Parameters>
	{
		...
		public static ValidationResult Validate(bool checkForWindows, ScrollRect scrollRect, Parameters parametersIfAlreadyCreated = null)
		{
...
#if OSA_SCROLLRECT_HAS_VIEWPORT
		...
		protected override void InitWithExistingParams()
		{
			...
			else if (_State == State.RECOMPILATION_SELECT_GENERATED_IMPLEMENTATION_PENDING) // this represents the previous state, so now PRE changes to POST
			{
				...
				{
					throw new UnityException("OSA: Internal error: _WindowParams.generatedScriptNameToUse is null after recompilation; " +
						"availableImplementations=" + implementationsString);
				}
				else if ((fullName = GetFullNameIfScriptExists(scriptName)) == null)
				{
					throw new UnityException("OSA: Internal error: Couldn't find the type's fullName for script '" + scriptName + "'. Did you delete the newly created script?\n " +
						"availableImplementations=" + implementationsString);
				}
				else
				{
					...
					if (index == -1)
					{
						throw new UnityException("OSA: Internal error: Couldn't find index of new implementation of '" + scriptName + "': " +
		...
		{
			var vr = Validate(false, _WindowParams == null ? null : _WindowParams.scrollRect, _WindowParams);
		...
		protected override void ConfigureScrollView(ScrollRect scrollRect, RectTransform viewport, params Transform[] objectsToSkipDisabling)
		{
			...
			Debug.Log("OSA: Starting with v4.0, the ScrollRect component is not needed anymore. It was disabled and you can safely remove it");
		}
		...
		{
			string nameWithoutController = controllerPrefab.name.Replace("Controller", "");
		...
		{
			var instanceScollbarFixer = _WindowParams.scrollbar.GetComponent<ScrollbarFixer8>();
			if (!instanceScollbarFixer)
				instanceScollbarFixer = _WindowParams.scrollbar.gameObject.AddComponent<ScrollbarFixer8>();
			//instanceScollbarFixer.scrollRect = _WindowParams.scrollRect;
			instanceScollbarFixer.externalScrollRectProxy = iAdapter;
			instanceScollbarFixer.viewport = _WindowParams.viewportRT;
...
#region Serialization
			...
			public static Parameters Create(ValidationResult validationResult)
			{
